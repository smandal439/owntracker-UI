...
// Process GPS JSON Data - Updated for your JSON format
function processIncomingGPS(payloadData, receivedTopic = '') {
  ...
  try {
    ...
    // Extract battery status - looks for batt or battery field
...
// Update Map & Telemetry UI - Updated for battery support
...
// Refresh active device sidebar list - Updated with battery display
...
// Update UI Telemetry Card - Updated with battery
function updateTelemetryUI(dev) {
  ...
  valBattery.innerText = dev.battery !== undefined ? dev.battery + '%' : '--%';
...
function startSimulator() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    simDevices.push({
      ...
      battery: Math.floor(Math.random() * 40 + 60) // 60-100% battery for sim
    });
  }
  ...
  simIntervalId = setInterval(() => {
    ...
    simDevices.forEach(simDev => {
      ...
      const payloadObj = {
        ...
        batt: simDev.battery // Add battery to simulated data
      };
